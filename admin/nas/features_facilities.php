...
if (isset($_POST['del_features'])) {
    ...
    $check_q= select('SELECT * FROM `room_features` WHERE `features_id`=?',[$frm_data['del_features']],'i');
...
if (isset($_POST['del_facility'])) {
    ...
    $check_q= select('SELECT * FROM `room_facilities` WHERE `facilities_id`=?',[$frm_data['del_facility']],'i');
